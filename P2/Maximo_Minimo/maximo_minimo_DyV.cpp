 ...
 */
Resultado encontrar_maximo_minimo_segmento(int *v, int inicio, int fin) {
    Resultado res;
    int min = v[0];
    int max = v[0];
    int idx_max = 0;
...
void encontrar_maximo_minimo(int *v, int n, int &max_idx, int &min_idx) {
    ...
    #pragma omp parallel sections
    {
        #pragma omp section
        {
            Resultado res_izq = encontrar_maximo_minimo_segmento(v, 0, medio);
        ...
        #pragma omp section
        {   
            Resultado res_der = encontrar_maximo_minimo_segmento(v, medio, n);
